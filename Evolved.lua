...
function saveProxyUsage(proxy_usage)
    ...
    if file then
        ...
        printm("[Ошибка] Не удалось сохранить статистику по прокси.")
    end
end
...
function checkProxyLimit(proxy_ip)
    ...
    if proxy_usage[proxy_ip] then
        if proxy_usage[proxy_ip].count >= 2 then
            printm("[Ошибка] Превышено максимальное количество подключений для IP: " .. proxy_ip)
...
function connect_random_proxy()
    ...
    if checkProxyLimit(my_proxy_ip) then
        ...
        printm("[Ошибка] Подключение с этим прокси невозможно из-за ограничения на количество подключений.")
    end
end
...
local function loadAllowedSerials()
    ...
    if response.status_code == 200 then
        ...
        if data and data.allowed_serials then
            ...
            printm("[Ошибка] Формат данных с GitHub некорректен.")
            return nil
        end
    else
        printm("[Ошибка] Не удалось загрузить файл с разрешенными серийными номерами.")
        return nil
    end
end
...
local function saveSerialsToFile(serials)
    ...
    if not file then
        printm("[Ошибка] Не удалось открыть файл для записи серийных номеров.")
        return
    end
...
local function addSerialToFile(serial)
    ...
    for _, existingSerial in ipairs(serials) do
        if existingSerial == serial then
            printm("Серийный номер уже сохранен.")
    ...
    printm("Серийный номер добавлен и сохранен.")
end
...
local function writeVersion(newVersion)
    ...
    if not file then
        printm("[Ошибка] Не удалось открыть файл для записи версии.")
        return
    end
...
local function getRemoteVersion()
    ...
    if response.status_code == 200 then
        ...
        printm("[Ошибка] Не удалось загрузить файл версии.")
        return nil
    end
end
...
function autoUpdate()
    ...
    if not remoteVersion then
        printm("[Ошибка] Не удалось получить версию с удалённого источника.")
        return
    end

    printm(string.format("[Обновление] Удалённая версия: %s, Локальная версия: %s", remoteVersion, CURRENT_VERSION))
    ...
    if isVersionNewer(remoteVersion, CURRENT_VERSION) then
        ...
        if response.status_code == 200 then
            ...
            printm(string.format("[Обновление] Обновление завершено: новая версия %s установлена. Перезагрузите скрипт.", remoteVersion))
        else
            printm("[Ошибка] Не удалось загрузить обновлённый скрипт.")
        end
    else
        printm("[Обновление] Установлена последняя версия скрипта.")
    end
end
...
if checkIfSerialAllowed(currentSerial) then
    printm("Серийный номер разрешен.")
else
    printm("Серийный номер не разрешен, выполнение скрипта приостановлено.")
...
function onLoad()
    ...
    printm('\x1b[0;36m------------------------------------------------------------------------\x1b[37m')
    printm('')

    printm('			\x1b[0;33m        EVOLVED\x1b[37m  - \x1b[0;32mАКТИВИРОВАН\x1b[37m           ')
    printm('           \x1b[0;33m        Made for AMARAYTHEN    by      vk.com/hentaikazz    \x1b[37m                                         ')
    printm('')
    printm('\x1b[0;36m------------------------------------------------------------------------\x1b[37m')
end
...
function generatenick()
    ...
    setBotNick(nick)
	printm('[\x1b[0;33mEVOLVED\x1b[37m] \x1b[0;36mИзменили ник на: \x1b[0;32m'..getBotNick()..'\x1b[37m.')
	reconnect(1)
end
...
local function writeToFile(fileName, text)
    ...
    if file then
        ...
        printm("Не удалось открыть файл для записи: " .. fileName)
    end
end
...
local function checkAndWriteLevel()
    while true do
        ...
        printm("Текущий уровень: " .. score)
        printm("Необходимый уровень: " .. requiredLevel)

        -- Проверка уровня
        if score >= requiredLevel then
            printm("Уровень достаточен, записываю в файл...")  -- Логируем запись в файл
            writeToFile("config\\accounts.txt", ("%s | %s | %s | %s | %s"):format(getBotNick(), tostring(cfg.main.password), score))
            generatenick()
        else
            printm("Уровень недостаточен для записи.")
        end
...
function sampev.onShowDialog(id, style, title, btn1, btn2, text)
    newTask(function()
        ...
        if id == 4423 then 
            ...
            printmm("Устроился на работу грузчика!")
            gruzchik()
            return false
        end
    end)
...
-- грузщики 
function printmm(text)
	printm("\x1b[0;36m[EVOLVED]:\x1b[37m \x1b[0;32m"..text.."\x1b[37m")
end
...
function nagruz()
    newTask(function()
        ...
        updateSync()
        printmm("Телепортируюсь работать грузчиком.")
...
function gruzchik()
    ...
    printmm("Взял мешок с зерном. Жду 15 секунд перед следующим тп для того что-бы не кикнуло.")
    wait(15000)
    tp(2163.060546875, -2238.1853027344, 13.287099838257)
    wait(3333)
    tp(2167.7253417969, -2262.1433105469, 13.30480670929)
    printmm("Отнёс мешок с зерном. Жду 15 секунд перед следующим тп для того что-бы не кикнуло.")
    wait(15000)
    -- 2 круг
    tp(2225.4377441406, -2276.4077148438, 14.764669418335)
    wait(3333)
...
local function readCoordsFromFile(filePath)
    ...
    if not file then
        printm("[Ошибка] Не удалось открыть файл: " .. filePath)
        return coords
    end
...
local function getRandomCoord(coords)
    if #coords == 0 then
        printm("[Ошибка] Список координат пуст.")
        return nil
    end
...
local function teleportToRandomLocation()
    if teleportActive then
        printm("[Ошибка] Телепортация уже выполняется, невозможно запустить новую.")
        return
    end
    ...
    newTask(function() -- Создаём корутину для телепортации
        ...
        if randomCoord then
            ...
            printm(string.format("[INFO] Телепортируемся в: tp(%.13f, %.13f, %.13f)", x, y, z))

            -- Вызов функции tp(x, y, z)
            tp(x, y, z)
        else
            printm("[Ошибка] Координаты не найдены, телепортация невозможна.")
        end
...
function sampev.onSendSpawn()
    newTask(function()
        ...
        if cfg.main.runspawn == 1 then
            ...
            printmm("[INFO] Побег со спавна отключен.")
        end
    end)
end
